...
public class ZCrashHandler implements Thread.UncaughtExceptionHandler {
    ...
    private static final String TAG = "ZCrashHandler";

    //系统默认的UncaughtException处理类
    private Thread.UncaughtExceptionHandler mLocalExceptionHandler;
    //CrashHandler实例
    private static ZCrashHandler mInstance = null;
    //程序的Context对象
    ...
    public static ZCrashHandler getInstance() {
        synchronized (ZCrashHandler.class) {
            if (null == mInstance) {
                mInstance = new ZCrashHandler();
            }
        }
        return mInstance;
    }
