package com.lecarlink.zframwork.utils.collection;
...
public class ZCollectionPicker {
    private static final String TAG = ZCollectionPicker.class.getSimpleName();
    ...
    public static <T extends ABICollectionPicker<T>> T pickFirst(T expect, Collection<T> collection) {
        if (null == expect || ZTextUtil.isEmpty(collection)) {
            return null;
        }
        T picked = null;
        for (T t : collection) {
