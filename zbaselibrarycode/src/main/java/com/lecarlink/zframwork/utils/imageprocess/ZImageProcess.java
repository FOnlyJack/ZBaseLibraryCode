...
import com.lecarlink.zframwork.utils.ZIOUtil;
import com.lecarlink.zframwork.utils.ZViewUtil;
import com.lecarlink.zframwork.utils.imageprocess.blur.FastBlur;

import java.io.*;


public class ZImageProcess {
    public static final String TAG = ZImageProcess.class.getSimpleName();
...
//        canvas.translate(-view.getLeft()/scaleFactor, -view.getTop()/scaleFactor);
    ...
    public static void fastBlurSetBg(Context context, Bitmap bm, View view, boolean downScale, float scaleFactor) {
        ...
        ZViewUtil.setBackgroundDrawable(view, new BitmapDrawable(context.getResources(), overlay));
    }
    /***************************************BLUR END****************************************/
...
//        float ww = 480f;//这里设置宽度为480f
        ...
        try {
            ...
            ZIOUtil.closeIO(isBm, os);
            ZIOUtil.recycleBitmap(bitmap);
        }
...
//		Bitmap originalBitmap = drawableToBitmap(drawable);
    ...
    public static Bitmap formatCameraPictureOriginal(String path, Bitmap bitmap) {
         ...
         */
        int degree = ZImageProcess.readPictureDegreeFromExif(path);
        if (0 == degree) {
            return bitmap;
        }
        /**
